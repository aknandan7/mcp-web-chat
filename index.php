...
function markActiveSession(li){
    ...
    $('#chatSessionsList li strong .live-dot').remove();
        ...
        success:function(res){
            if(res.status==='success'){
                ...
                res.sessions.forEach((session, index) => {
                    ...
                    let li = $(`
                        <li data-id="${session.id}" ${index===0 ? 'class="active-session"' : ''}>
                            <strong>${title}${index===0 ? '<span class="live-dot"></span>' : ''}</strong><br>
                            <small style="font-size:10px;color:#181717c7;margin-top:0px;position:absolute;">
                                ${session.created_at}
                            </small>
                        </li>
                    `);
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            if(res.status==='success'){
                ...
                if(history.length===0){ $('#chatBox').html('<div style="text-align:center;color:#666;margin-top:20px;">No chat history yet</div>'); }
