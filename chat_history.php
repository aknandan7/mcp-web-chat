...
$user = "root";
$pass = "root";
$dbname = "indovis1_emp_v15"; // <-- change to your DB name
...
$sessionQuery = $conn->prepare("SELECT id, indo_code, title, created_at FROM chat_sessions WHERE id = ?");
...
while ($row = $historyResult->fetch_assoc()) {
    $history[] = [
        "id" => $row['id'],
        "sender" => $row['message_from'], // "user" or "bot"
        "message" => $row['message'],
